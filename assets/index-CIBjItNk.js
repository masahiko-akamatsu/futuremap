...
`)};function ts(a){a.onreadystatechange&&a.onreadystatechange.call(a)}Object.defineProperty(zo.prototype,"withCredentials",{get:function(){return this.m==="include"},set:function(a){this.m=a?"include":"same-origin"}});function Hf(a){let c="";return O(a,function(d,p){c+=p,c+=":",c+=d,c+=`\r
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class Lt{constructor(e,n){this.bucket=e,this.path_=n}get path(){return this.path_}get isRoot(){return this.path.length===0}fullServerUrl(){const e=encodeURIComponent;return"/b/"+e(this.bucket)+"/o/"+e(this.path)}bucketOnlyServerUrl(){return"/b/"+encodeURIComponent(this.bucket)+"/o"}static makeFromBucketSpec(e,n){let r;try{r=Lt.makeFromUrl(e,n)}catch{return new Lt(e,"")}if(r.path==="")return r;throw DC(e)}static makeFromUrl(e,n){let r=null;const i="([A-Za-z0-9.\\-_]+)";function s(D){D.path.charAt(D.path.length-1)==="/"&&(D.path_=D.path_.slice(0,-1))}const o="(/(.*))?$",l=new RegExp("^gs://"+i+o,"i"),u={bucket:1,path:3};function h(D){D.path_=decodeURIComponent(D.path)}const f="v[A-Za-z0-9_]+",g=n.replace(/[.]/g,"\\."),_="(/([^?#]*).*)?$",w=new RegExp(`^https?://${g}/${f}/b/${i}/o${_}`,"i"),P={bucket:1,path:3},N=n===lE?"(?:storage.googleapis.com|storage.cloud.google.com)":n,b="([^?#]*)",I=new RegExp(`^https?://${N}/${i}/${b}`,"i"),k=[{regex:l,indices:u,postModify:s},{regex:w,indices:P,postModify:h},{regex:I,indices:{bucket:1,path:2},postModify:h}];for(let D=0;D<k.length;D++){const M=k[D],O=M.regex.exec(e);if(O){const v=O[M.indices.bucket];let m=O[M.indices.path];m||(m=""),r=new Lt(v,m),M.postModify(r);break}}if(r==null)throw NC(e);return r}}class bC{constructor(e){this.promise_=Promise.reject(e)}getPromise(){return this.promise_}cancel(e=!1){}}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
`).map(I=>I.replace(/^[🔲□■・]/,"").trim()).filter(Boolean):[];return{...w,tasks:b}}),h=u.find(w=>w.id==="health"),f=[];return(_=h==null?void 0:h.tasks)!=null&&_.length&&h.tasks.forEach(w=>{const P=w.indexOf("："),N=w.indexOf(":"),b=P>=0?P:N;b>=0?f.push({label:w.slice(0,b).trim(),value:w.slice(b+1).trim()}):f.push({label:w,value:""})}),{title:n,theme:s,emotionTags:o,sections:u,healthItems:f,isMonthly:r,catMap:i}}function ix(){const[t,e]=J.useState(void 0),[n,r]=J.useState("monthly"),[i,s]=J.useState(null),[o,l]=J.useState(!1),[u,h]=J.useState({show:!1,msg:""}),[f,g]=J.useState(!1),[_,w]=J.useState(!1),[P,N]=J.useState(!1),b=J.useRef(null),I=new Date,[y,k]=J.useState(I.getFullYear()),[D,M]=J.useState(I.getMonth()+1);J.useEffect(()=>Z1(Ls,R=>e(R??null)),[]),J.useEffect(()=>{if(!t)return;s(null);const R=`${t.uid}_${y}_${String(D).padStart(2,"0")}`;vC(xg(Mg,"futuremaps",R)).then(T=>{s(T.exists()?T.data():nx(y,D))})},[t,y,D]);const O=J.useCallback(async R=>{if(!t||!i)return;l(!0);const T=`${t.uid}_${y}_${String(D).padStart(2,"0")}`,pe={...i,...R,uid:t.uid,updatedAt:SC()};try{await wC(xg(Mg,"futuremaps",T),pe),s(pe)}catch(at){console.error(at)}l(!1)},[t,i,y,D]),v=(R,T)=>{const pe=`${R}:${T}`,at={...i.checked,[pe]:!i.checked[pe]};if(O({checked:at}),!i.checked[pe]){const rn=Fg[Math.floor(Math.random()*Fg.length)];h({show:!0,msg:rn}),clearTimeout(b.current),b.current=setTimeout(()=>h(Ct=>({...Ct,show:!1})),2600)}},m=R=>{O({openSecs:{...i.openSecs,[R]:!i.openSecs[R]}})},E=R=>{R===-1?D===1?(k(T=>T-1),M(12)):M(T=>T-1):D===12?(k(T=>T+1),M(1)):M(T=>T+1)},S=J.useCallback(R=>{var pe,at;const T={sections:R.sections,checked:{},openSecs:{}};R.theme&&(T.theme=R.theme),(pe=R.emotionTags)!=null&&pe.length&&(T.emotionTags=R.emotionTags),(at=R.healthItems)!=null&&at.length&&(T.health=R.healthItems.length>0?R.healthItems:(i==null?void 0:i.health)||[]),O(T),N(!1),h({show:!0,msg:"📊 Excelからインポートしました！"}),clearTimeout(b.current),b.current=setTimeout(()=>h(rn=>({...rn,show:!1})),3e3)},[O,i]);if(t===void 0)return C.jsx(jg,{msg:"読み込み中..."});if(t===null)return C.jsx(lx,{});if(n==="monthly"&&!i)return C.jsx(jg,{msg:"データを取得中..."});const A=[{id:"monthly",label:"📅 毎月"},{id:"1year",label:"🌱 1年後"},{id:"3year",label:"🌿 3年後"},{id:"10year",label:"🌳 10年後"}];return C.jsxs("div",{style:{fontFamily:"'Zen Maru Gothic',sans-serif",background:"#f1f8f4",minHeight:"100vh",color:"#1a3c2a",maxWidth:640,margin:"0 auto"},children:[C.jsx("style",{children:`
