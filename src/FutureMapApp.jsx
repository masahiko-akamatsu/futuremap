...
export default function FutureMapApp({user}){
    ...
    const load=async()=>{
      ...
      try{
        ...
        if(snap.exists()){
          ...
          if(snap.data().periods) setPeriods({...DEFAULT_PERIODS,...snap.data().periods});
        } else {
          const initData={...INITIAL_DATA,[todayKey]:INITIAL_DATA.month}; delete initData.month;
          setData(initData); setMonthKeys([todayKey]);
        }
      }catch(e){
        console.error(e);
        const initData={...INITIAL_DATA,[todayKey]:INITIAL_DATA.month}; delete initData.month;
  ...
  const saveToFirestore=useCallback(async(newData,newPeriods)=>{
    setSaving(true);
    try{
      const ref=doc(db,'users',user.uid,'futuremap','data');
      const payload={content:newData,updatedAt:serverTimestamp()};
      if(newPeriods) payload.periods=newPeriods;
  ...
  const updateData=(tabId,key,value)=>{
    setData(prev=>{
      ...
      const t=setTimeout(()=>saveToFirestore(next,null),1500);
      setSaveTimer(t);
      return next;
    });
  };
  ...
  const savePeriod=()=>{
    ...
    setData(prev=>{saveToFirestore(prev,newP);return prev;});
  };
  ...
  const addMonth=(sourceKey)=>{
    ...
    const sd=sourceKey?{...(data[sourceKey]||{})}:{};
    setData(prev=>{
  ...
  const handleExport=async()=>{
    setExporting(true);
    try{const ed={...data,month:data[activeMonthKey]||{}};await exportToExcel(ed,user.displayName||user.email);}
  ...
  const PeriodBadge=({tabId:tid,isActive})=>{
    if(editingPeriod===tid){
      return <input ref={periodInputRef} value={periodInput} onChange={e=>setPeriodInput(e.target.value)} onBlur={savePeriod} onKeyDown={e=>{if(e.key==='Enter')savePeriod();if(e.key==='Escape')setEditingPeriod(null);}} onClick={e=>e.stopPropagation()} style={{fontSize:'10px',padding:'2px 6px',borderRadius:'6px',border:'none',outline:'2px solid rgba(255,255,255,0.9)',background:'rgba(255,255,255,0.95)',color:GREEN[800],fontFamily:'inherit',width:'115px',fontWeight:'500'}}/>;
    }
    return <span onClick={e=>{e.stopPropagation();startEditPeriod(tid);}} title="クリックして日付を編集" style={{padding:'2px 7px',borderRadius:'10px',fontSize:'10px',fontWeight:'400',background:isActive?'rgba(46,125,80,0.12)':'rgba(255,255,255,0.15)',color:isActive?GREEN[600]:'rgba(255,255,255,0.7)',cursor:'text',borderBottom:isActive?'1px dashed '+GREEN[400]:'1px dashed rgba(255,255,255,0.45)',transition:'all 0.15s'}} onMouseOver={e=>e.currentTarget.style.opacity='0.7'} onMouseOut={e=>e.currentTarget.style.opacity='1'}>{periods[tid]||DEFAULT_PERIODS[tid]} ✎</span>;
  };
      ...
      <header style={{background:'linear-gradient(135deg,'+GREEN[800]+' 0%,'+GREEN[600]+' 60%,'+GREEN[400]+' 100%)',padding:'0',position:'sticky',top:0,zIndex:50,boxShadow:'0 2px 12px rgba(0,0,0,0.2)'}}>
        ...
        <div style={{display:'flex',gap:'2px',padding:'10px 24px 0',overflowX:'auto',alignItems:'flex-end'}}>
          {monthKeys.map(mk=>{
            ...
            return(<button key={mk} onClick={()=>{setCurrentTab(0);setActiveMonthKey(mk);}} style={{padding:'8px 14px 10px',border:'none',borderRadius:'9px 9px 0 0',fontFamily:'inherit',fontSize:'12px',fontWeight:isActive?'700':'400',cursor:'pointer',whiteSpace:'nowrap',background:isActive?'#f4f8f5':'rgba(255,255,255,0.12)',color:isActive?GREEN[800]:'rgba(255,255,255,0.8)'}}>
          ...
          <button onClick={()=>{setNewYear(now.getFullYear());setNewMonth(now.getMonth()+1);setShowAddModal(true);}} style={{padding:'7px 13px 9px',border:'1px dashed rgba(255,255,255,0.5)',borderBottom:'none',borderRadius:'9px 9px 0 0',fontFamily:'inherit',fontSize:'13px',fontWeight:'700',cursor:'pointer',background:'rgba(255,255,255,0.1)',color:'rgba(255,255,255,0.9)',marginLeft:'2px'}}>＋</button>
          ...
          {TABS.slice(1).map((t,i)=>{
            ...
            return(<button key={t.id} onClick={()=>setCurrentTab(i+1)} style={{padding:'9px 14px 11px',border:'none',borderRadius:'9px 9px 0 0',fontFamily:'inherit',fontSize:'13px',fontWeight:isActive?'700':'500',cursor:'pointer',whiteSpace:'nowrap',background:isActive?'#f4f8f5':'rgba(255,255,255,0.12)',color:isActive?GREEN[800]:'rgba(255,255,255,0.75)',display:'flex',alignItems:'center',gap:'6px'}}>
              {t.label}<PeriodBadge tabId={t.id} isActive={isActive}/>
            </button>);
          })}
        </div>
      </header>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'16px'}}>
          {isMonthTab?(
            ...
            {editingPeriod===tab.id?(
              <input ref={periodInputRef} value={periodInput} onChange={e=>setPeriodInput(e.target.value)} onBlur={savePeriod} onKeyDown={e=>{if(e.key==='Enter')savePeriod();if(e.key==='Escape')setEditingPeriod(null);}} style={{fontSize:'13px',padding:'2px 8px',borderRadius:'6px',border:'1px solid '+GREEN[400],outline:'none',fontFamily:'inherit',color:GREEN[800],width:'140px'}}/>
            ):(
              <span onClick={()=>startEditPeriod(tab.id)} title="クリックして日付を編集" style={{fontSize:'13px',color:GREEN[600],cursor:'text',borderBottom:'1px dashed '+GREEN[400],paddingBottom:'1px'}}>{periods[tab.id]||DEFAULT_PERIODS[tab.id]} ✎</span>
            )}</>
          )}
        ...
        <div onClick={()=>setShowAddModal(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'18px',width:'100%',maxWidth:'440px',boxShadow:'0 20px 60px rgba(0,0,0,0.25)',overflow:'hidden'}}>
            <div style={{padding:'18px 20px 16px',background:'linear-gradient(135deg,'+GREEN[800]+','+GREEN[600]+')',display:'flex',alignItems:'center',gap:'10px'}}>
              ...
              <div><div style={{fontSize:'16px',fontWeight:'700',color:'#fff'}}>月のアクションプランを追加</div><div style={{fontSize:'11px',color:'rgba(255,255,255,0.65)',marginTop:'2px'}}>年月を選んで作成してください</div></div>
            ...
            <div style={{padding:'22px 20px 20px'}}>
              ...
              <div>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                  {copyOpts.map(mk=>{const{year:y,month:m}=parseMonthKey(mk);return(<button key={mk} onClick={()=>addMonth(mk)} style={{padding:'11px 16px',border:'1px solid '+GREEN[400],borderRadius:'10px',background:'rgba(76,175,120,0.04)',color:GREEN[800],fontSize:'14px',fontWeight:'600',cursor:'pointer',fontFamily:'inherit',textAlign:'left',display:'flex',alignItems:'center',gap:'10px'}} onMouseOver={e=>{e.currentTarget.style.background='rgba(76,175,120,0.1)';e.currentTarget.style.borderColor=GREEN[600];}} onMouseOut={e=>{e.currentTarget.style.background='rgba(76,175,120,0.04)';e.currentTarget.style.borderColor=GREEN[400];}}>
                  ...
                  <button onClick={()=>addMonth(null)} style={{padding:'11px 16px',border:'1px solid #e0e0e0',borderRadius:'10px',background:'#fff',color:'#777',fontSize:'14px',fontWeight:'500',cursor:'pointer',fontFamily:'inherit',textAlign:'left',display:'flex',alignItems:'center',gap:'10px'}} onMouseOver={e=>e.currentTarget.style.background='#f8f8f8'} onMouseOut={e=>e.currentTarget.style.background='#fff'}><span>📝</span><span>白紙から作成する</span></button>
                </div>
              </div>
            </div>
          </div>
